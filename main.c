...
int main() {
	...
	SDL_Texture* backgroundTexture = loadTexture(renderer, "images/menu_b.bmp");
	const char* gameBackground = "images/level0.bmp";
	const char* gameMusic = "audio/level0.wav";
	SDL_RenderClear(renderer);
	SDL_RenderPresent(renderer);
	SDL_Delay(300);

	SDL_Texture* startButtonTexture = loadTexture(renderer, "images/start.bmp");	
	SDL_Texture* exitButtonTexture = loadTexture(renderer, "images/quit.bmp");
	SDL_Texture* volumeOn = loadTexture(renderer, "images/vol1.bmp");
	SDL_Texture* volumeOff = loadTexture(renderer, "images/vol2.bmp");
	...
	backgroundMusic = Mix_LoadMUS("audio/menu.wav");
	if (!backgroundMusic) {
